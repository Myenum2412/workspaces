...
import type { OrgMember, UserProfile } from "@/lib/appwrite/types";
...
export default function OrgUsersPage() {
  ...
  const { data: members = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await databases.listDocuments(DB_ID, COLLECTIONS.ORG_MEMBERS, [
          Query.equal("organizationId", orgId!),
          Query.limit(100),
        ]);
        return res.documents as unknown as OrgMember[];
      } catch {
        return [];
      }
    },
  ...
  const filteredRows = React.useMemo(() => {
    ...
    if (filterStatus !== "all") {
      result = result.filter(
        (row) => (row.member?.status ?? "pending") === filterStatus
      );
    }
    ...
    return result.filter(
      (row) =>
        `${row.profile.firstName} ${row.profile.lastName} ${row.profile.email} ${row.member?.role ?? ""}`
          .toLowerCase()
          .includes(query)
    );
    ...
    return [...filteredRows].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        case "name":
          ...
          break;
        case "role":
          aValue = a.member?.role ?? "";
          bValue = b.member?.role ?? "";
          break;
        case "status":
          ...
          break;
        case "joined":
          aValue = a.member?.joinedAt ?? "";
          bValue = b.member?.joinedAt ?? "";
          break;
        default:
          aValue = "";
          bValue = "";
  ...
  const toggleAll = () => {
    if (selectedIds.size === paginatedRows.length && paginatedRows.length > 0) {
      ...
      setSelectedIds(new Set(paginatedRows.map((row) => row.profile.$id)));
    }
  };
  ...
  const handleUserAdded = () => refresh();
  const handleUserDeleted = () => refresh();
  ...
  const SortIcon = ({ columnKey }: { columnKey: string }) => {
    ...
    if (sortConfig.direction === "asc")
      return <ArrowUpIcon className="size-3.5 text-slate-600" />;
    if (sortConfig.direction === "desc")
      return <ArrowDownIcon className="size-3.5 text-slate-600" />;
            ...
            {selectedIds.size > 0 && (
              <div className="mb-4 flex items-center justify-between rounded-xl bg-emerald-900 p-4 text-white">
                ...
                <Button
                  ...
                  className="text-emerald-100 hover:bg-emerald-800"
                >
                  Clear selection
                </Button>
              </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-emerald-50/70 hover:bg-emerald-50/70">
                    ...
                    <TableHead
                      className="px-4 py-4 font-semibold text-emerald-950 cursor-pointer group"
                  ...
                  {paginatedRows.map(({ profile, member }) => (
                    <TableRow
                      ...
                      className={cn(
                        ...
                        selectedIds.has(profile.$id) && "bg-slate-50/50"
                      )}
                      onClick={() => {
                        setSelectedUser({
                          ...profile,
