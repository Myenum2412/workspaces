  ...
  return (
    <section className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-600">Collections and indexes used by backend staff workflows.</p>
      </div>
        ...
        <div className="divide-y">
          {collections.map((collection) => (
            <div key={collection.name} className="grid gap-3 px-5 py-4 text-sm md:grid-cols-3">
              ...
              <span className="text-zinc-600">{collection.key}</span>
              <span className="text-zinc-600">{collection.purpose}</span>
            </div>
          ))}
        </div>
      </div>
