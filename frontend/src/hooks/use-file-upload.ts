...
export const useFileUpload = (options: FileUploadOptions = {}): [FileUploadState, FileUploadActions] => {
  ...
  // Keep track of previously processed files to identify specifically "added" files
  const prevFilesRef = useRef<FileWithPreview[]>(state.files);

  // Synchronize files change
  useEffect(() => {
    if (state.files !== prevFilesRef.current) {
      onFilesChange?.(state.files);

      // Identify newly added files
      const newFiles = state.files.filter(
        (file) => !prevFilesRef.current.some((prev) => prev.id === file.id)
      );
