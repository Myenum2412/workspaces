      ...
      {messages.map((msg) => {
        ...
        return (
          <div key={msg.id} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[70%] rounded-lg px-4 py-2 ${isOwn ? "bg-emerald-600 text-white" : "bg-zinc-100 text-zinc-900"}`}>
              {!isOwn && (
                <p className="mb-1 text-xs font-medium opacity-70">{msg.senderName || "Unknown"}</p>
              )}
              <p className="text-sm whitespace-pre-wrap">{msg.content}</p>
              <p className={`mt-1 text-right text-xs ${isOwn ? "text-emerald-100" : "text-zinc-500"}`}>
